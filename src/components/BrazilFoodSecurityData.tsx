...
export function BrazilFoodSecurityData({ data }: { data: NonNullable<NationalData['brazilFoodSecurity']> }) {
  ...
  return <article className="national-data__indicator">
    <h4>Segurança alimentar — Brasil (IBGE/PNAD Contínua)</h4>
    ...
    {latest && <p className="meta">Último ano disponível: {latest.year}. Os percentuais têm como denominador os domicílios particulares entrevistados.</p>}
    <p>A pesquisa aplica a Escala Brasileira de Insegurança Alimentar (EBIA) na PNAD Contínua. O indicador identifica a proporção de domicílios que enfrentam preocupação ou restrição no acesso regular a alimentos, separando os níveis moderado e grave.</p>
    ...
    <details><summary>Ver histórico da PNAD Contínua ({data.points.length} anos)</summary>
      <div className="national-data__table"><table><caption>Segurança alimentar no Brasil: percentual de domicílios</caption>
        <thead><tr><th scope="col">Ano</th><th scope="col">Alguma insegurança</th><th scope="col">Moderada</th><th scope="col">Grave</th></tr></thead>
        <tbody>{[...data.points].reverse().map((point) => <tr key={point.year}><td>{point.year}</td><td>{percentage(point.foodInsecurity)}</td><td>{percentage(point.moderate)}</td><td>{percentage(point.severe)}</td></tr>)}</tbody>
      </table></div>
