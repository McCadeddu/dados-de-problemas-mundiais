...
describe('WorkMigrationComparisonPanel', () => {
  it('uses one common year, reports excluded countries and follows the continent filter', () => {
    ...
    expect(within(screen.getByRole('table')).queryByText('País D')).not.toBeInTheDocument()
    ...
    expect(within(screen.getByRole('table')).getByText('País D')).toBeInTheDocument()
    ...
    expect(screen.getAllByText('Não calculável')).toHaveLength(4)
    expect(within(screen.getByRole('table')).queryByText('País A')).not.toBeInTheDocument()
  })
